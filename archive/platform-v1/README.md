...
| `tests/integration/milestone1_smoke_test.py`, `milestone2_smoke_test.py`, `milestone3_smoke_test.py`, `knowledge_capture_smoke_test.py`, `domain_materialization_smoke_test.py`, `business_approval_smoke_test.py`, `real_terraform_smoke_test.py` | Vertical Slice v0.1 integration smoke tests | All explicitly self-described as "Vertical Slice v0.1" milestones (a different, earlier milestone numbering than the current Execution Framework's) and require the now-archived `platform-api` container to run at all. `tests/integration/milestone5_smoke_test.py` is genuinely current-architecture (MCP Server, Execution-Framework.md §6) and was **not** archived. |

...
| **Nautobot** | Source of Truth for network inventory and topology (ADR-001, scoped by ADR-019) | Owns stage 1's structured-state write and stage 2's referential Validation; the domain generator reads it to produce the NetAsCode YAML that is the Execution Framework's actual Intent artifact (ADR-018); also the durable read target for stage 6's Verification write-back. Per ADR-019, Nautobot owns **Truth #2 (Desired State)** for network-shaped domains — not **Truth #1 (Business Intent)**, which is the AI/LangGraph layer's future responsibility | **Structured retrieval target** — queried live via GraphQL/REST, never duplicated into the knowledge base (existing boundary rule, unchanged) |
...
| **LangGraph (future)** | Not yet built | Not an owner of any stage — a cross-cutting reasoning layer that *reads* stage outputs (Nautobot state, GitLab pipeline/job status, this knowledge base, OPA decision traces) to answer engineer questions and *writes* only through the same Intent stage (stage 1) every other client uses. **Per [ADR-019](../adr/ADR-019-Three-Truths-Principle.md), LangGraph is also the future owner of Truth #1 (Business Intent)** — it will be responsible for translating business-level requests ("create a PCI payment zone") into sequences of domain-specific MCP tool calls, coordinating across domain SoTs when multiple domains are involved, without the MCP Server itself ever becoming an orchestration engine | **N/A — this is the future consumer being designed for**, not a data source |

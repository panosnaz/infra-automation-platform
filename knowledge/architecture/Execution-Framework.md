...
2. **The active deployment uses the repository-local isolated Nautobot stack.** Nautobot, GitLab, Vault, MinIO, OPA, MCP Server, and the GitLab Runner use the local isolated deployment and its explicit Docker network. Host-published endpoints remain available for local development clients; no nested or independently managed Nautobot Docker repository is part of the active platform.

...
| API Token | `<redacted — see secret/lab/nautobot in Vault>` |
...
| Workflow Orchestration | n8n | 🟡 Partial — Milestone 3 + Business Approval + Real Terraform Integration (M6A) complete: `RequestDeployment` → `PENDING_APPROVAL`/`ACCEPTED` → `DEPLOYING` (real `terraform init/plan/apply`) → `VALIDATING` → `STABLE`/`FAILED` proven end-to-end against the live APIC, including ADR-015's approve/deny gate for production; real n8n (Workflow Engine orchestration itself) and real validation (pyATS wired into the pipeline) remain unimplemented |
...
| 23 | Vertical Slice v0.1 (Control Plane proof) | New | ✅ Done | Milestone 1 ✅, Milestone 2 ✅, Milestone 3 ✅, Business Approval (ADR-015) ✅, Domain Materialization ✅, Knowledge Capture (M5) ✅, Real Terraform Integration (M6A) ✅ Complete (2026-07-16) — see [`05-Operations/14-Vertical-Slice-v0.1-Roadmap.md`](../05-Operations/14-Vertical-Slice-v0.1-Roadmap.md), `tests/integration/`. `RequestDeployment` now drives a real `terraform init/plan/apply` against the live APIC, independently verified. Next work is further capability implementation (real Validation/pyATS, RBAC, additional domains), not further architecture review. |

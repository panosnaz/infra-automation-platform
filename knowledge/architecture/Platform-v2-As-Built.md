...
The reference architecture's Docker Networking Diagram (§3) describes three explicit named networks with deliberately chosen, non-conflicting subnets. The active local deployment is defined by the repository's root [`docker/docker-compose.yml`](../../docker/docker-compose.yml), which includes the repository-local isolated Nautobot stack and platform services under one Compose project with explicitly managed networks outside Docker's default auto-allocation pool.

**Two real issues were hit and fixed during this consolidation, both now documented as lessons in repo memory:**

1. **Project-name reconciliation risk.** Compose reconciles *all* resources sharing a project name during `up`/`down`, not just what's declared in the current invocation. The root file now includes the repository-local isolated Nautobot stack explicitly, so the active deployment has one complete local definition.
2. **Volume/network key collisions under `include:`.** Compose merges all included files' top-level `volumes:`/`networks:` keys into one flat namespace — generic local key names used by multiple files collide, with one silently winning. The active local stack uses explicit service and volume names to keep resources isolated.

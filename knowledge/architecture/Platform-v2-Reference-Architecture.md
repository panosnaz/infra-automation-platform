...
## 2. Nautobot is the Source of Truth

Nautobot owns desired state, inventory, topology, allocations, lifecycle, and engineering metadata. No desired state exists outside Nautobot — not in the MCP Server, not in GitLab, not in a JSONL file.
...
**Phase 5 — Decommission Platform v1 outright.** Delete `lab/docker/platform-api/` (`main.py`, `execution_store.py`, `approval_workflow.py`, `terraform_executor.py`, `validation_stub.py`, `nautobot_store.py`, and the already-dead `terraform_stub.py`). Relocate the reusable pieces (`aci_materializer.py`'s logic, `technical_policy.py`'s client shape, `jsonl_writer.py`, `CanonicalIntent`) into the MCP Server / GitLab CI as designed in §6-7. Rewrite the 7 integration tests' trigger mechanisms (assertions preserved, per the previously reviewed test-by-test mapping) to target Platform v2. No compatibility shim is built — this is the explicit replacement, not a bridge.

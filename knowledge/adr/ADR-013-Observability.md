...
>
> **Update (2026-08-19) — the Prometheus/Loki gap above is now closed:** Nautobot's `/metrics` scrape now works (Prometheus overrides its `Accept` header to `*/*`, working around a real Nautobot 3.1.4 content-negotiation incompatibility); the stale `platform-api` target was replaced with a working `opa` one (OPA exposes Prometheus metrics natively); and Loki — found during this same investigation to have **no log shipper configured at all** — now receives real container logs via a new `promtail` service. See [`Platform-v2-As-Built.md`](../architecture/Platform-v2-As-Built.md) §3 for full detail. GitLab CI jobs still only produce plain stdout (not yet shipped anywhere structured beyond Promtail picking up the container's stdout) — that part of the original gap remains open.
>
> **Original status (2026-06-30):** Not yet implemented (0%). No platform component currently emits metrics, structured logs, or traces. See [`Current-State-v1.md`](../architecture/archive/Current-State-v1.md) Gaps table. The generator, Terraform, and Ansible currently only produce plain stdout output.
>
...
> GitLab CI job logs and the Grafana/Loki/Prometheus stack referenced above are the real
> observability surface today, not a Platform API/Workflow Engine emitting to it.

...
**Status:** Accepted — schema, generator, Terraform module, and Ansible playbooks are complete; **live verification achieved on all 4 real nodes** — real `terraform apply` cycles succeeded against genuine Nexus 9000v hardware on all 4: `DC1-Leaf` (2026-07-30, including a `destroy`), `DC1-BGW` (2026-07-31, §10), and `DC2-Leaf`/`DC2-BGW` (2026-07-31, §11). pyATS EVPN tests are written and mock-validated (§12). A real destroy-safety bug was found in `nxos_feature` (see §6) and mitigated with `lifecycle { prevent_destroy = true }` (§7, 2026-07-31) — no attribute-level fix exists in the provider's real schema. Pipeline wiring remains blocked by a real GitLab Runner network-reachability gap (§12).

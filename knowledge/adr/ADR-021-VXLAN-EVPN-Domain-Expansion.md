...
**Status:** Accepted — schema, generator, Terraform module, and Ansible playbooks are complete; **live verification achieved on all 4 real nodes** — real `terraform apply` cycles succeeded against genuine Nexus 9000v hardware: `DC1-Leaf` (2026-07-30, including a `destroy`) and `DC1-BGW` (2026-07-31, §10); `DC2-Leaf`/`DC2-BGW` confirmed live via NX-API. A real destroy-safety bug was found in `nxos_feature` (see §6) and mitigated with `lifecycle { prevent_destroy = true }` (§7, 2026-07-31) — no attribute-level fix exists in the provider's real schema.
...
- **Live verification achieved on all 4 real nodes (2026-07-30/31)**: real `terraform apply` cycles succeeded against genuine Nexus 9000v hardware via a jump host inside the CML lab network — `DC1-Leaf` (§6, including a `destroy`) and `DC1-BGW` (§10); `DC2-Leaf`/`DC2-BGW` confirmed live via NX-API. This closes the core gap this ADR opened with. A real destroy-safety bug was found in `nxos_feature` (destroy doesn't actually revert device state), confirmed via the real provider schema to have no attribute-level fix available, and mitigated with `lifecycle { prevent_destroy = true }` (see §7) rather than left unaddressed.

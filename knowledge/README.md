...
| `runbooks/` | Operational procedures — repeatable, step-by-step. Domain-specific runbooks are prefixed with their domain (`ACI-Day2-Operations.md`, `CML-EVPN-Lab-Jump-Host.md`, `Demo-Rebuild-From-Nautobot.md`). One-time validation reports (evidence collected on a specific date, not a repeatable procedure) belong in `architecture/` instead, not here. |

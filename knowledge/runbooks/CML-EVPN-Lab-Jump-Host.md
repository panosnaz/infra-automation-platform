...
**Verify results independently of Terraform's own output** — query the device directly (via NX-API, as in step 5, or console) both after `apply` and after `destroy`. This caught a real bug: `terraform destroy` on `nxos_feature` reports success but does **not** actually revert the device's feature state (see ADR-021 §6 and `Platform-Status-and-Pending-Items.md` for the open tracking item). Never assume a reported-successful destroy actually happened — check the real device.
...
- **This is not wired into any CI pipeline.** Everything in this runbook is manual, interactive, console/API-driven verification — not yet automated as a repeatable GitLab CI job. See `Platform-Status-and-Pending-Items.md` for the current state of `pipelines/evpn.gitlab-ci.yml`.

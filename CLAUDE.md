...
**Domain Expansion Phase 2 — VXLAN EVPN ([ADR-021](knowledge/adr/ADR-021-VXLAN-EVPN-Domain-Expansion.md)):** the first domain expansion beyond ACI, proving the Execution Framework's mechanism generalizes with zero shared-pipeline-logic changes. Built the Terraform module (`platform/terraform/evpn/`, real `CiscoDevNet/nxos` provider), Nautobot Custom Fields, `pipelines/evpn.gitlab-ci.yml`, and 3 MCP tools. **Since updated — this is no longer simulator-blocked**: a real CML lab with 4 genuine Nexus 9000v devices was found and used. All 4 devices (`DC1-Leaf`, `DC1-BGW`, `DC2-Leaf`, `DC2-BGW`) now have a proven, live `terraform apply` cycle against real hardware, and pyATS tests for the domain are written and validated. What remains: the EVPN pipeline still isn't wired into the root GitLab pipeline, because the GitLab Runner has no network path to these devices (a lab networking limitation, not a code gap) — see ADR-021 §9/§13/§14 and [`Platform-Status-and-Pending-Items.md`](knowledge/architecture/Platform-Status-and-Pending-Items.md) for the current, exact state of that work.
